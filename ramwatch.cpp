				...
				default:
				{
					...
					switch (lP->code)
					{
						...
						{	
							...
							SETWINDOWL(hDlg, MSGRESULT_T, ListView_GetSelectionMark(GetDlgItem(hDlg,IDC_WATCHLIST)));
							return 1;
						}
					}
				}
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				case IDC_C_WATCH_EDIT:
					...
					DialogBoxParam(winClass.hInstance, MAKEINTRESOURCE(IDD_EDITWATCH), hDlg, (DLGPROC) EditWatchProc,(LPARAM) watchIndex);
					SetFocus(GetDlgItem(hDlg,IDC_WATCHLIST));
					return true;
				case IDC_C_WATCH:
					...
					DialogBoxParam(winClass.hInstance, MAKEINTRESOURCE(IDD_EDITWATCH), hDlg, (DLGPROC) EditWatchProc,(LPARAM) WatchCount);
					SetFocus(GetDlgItem(hDlg,IDC_WATCHLIST));
					return true;
